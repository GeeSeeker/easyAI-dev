...
2. `EASYAI_PROJECT_ROOT` 环境变量 — 用户手动指定的后备方案
...
1. 确认当前项目已执行 `npx @geeseeker/easyai-dev init .`（`.trellis/` 目录存在）
2. 如果 roots 自动检测失败，在 MCP 配置中添加环境变量作为后备：
   ```json
   "easyai-mcp-server": {
     "command": "npx",
     "args": ["-y", "@geeseeker/easyai-dev", "serve"],
