...
import { getConfig, getDefaultUser } from "./config-loader.js";
// ============ 常量 ============
 ...
 */
function getJournalDir(user = getDefaultUser()) {
    // 防止路径遍历攻击
 ...
 */
function getLatestJournalFile(user = getDefaultUser()) {
 ...
 * @param user - 用户名，默认为 'default'
 */
function appendJournalEntry(entry, user = getDefaultUser()) {
    const journalDir = getJournalDir(user);
    ...
    const maxLines = getConfig().journal.maxLinesPerFile;
    // 如果超过最大行数，创建新文件
 ...
 * @returns 匹配的日志条目列表
 */
function searchJournal(query, user = getDefaultUser()) {
    const journalDir = getJournalDir(user);
    if (!fs.existsSync(journalDir)) {
        return [];
    }
