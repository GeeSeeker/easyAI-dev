...
// ============ MCP Roots 项目根目录 ============
/**
 * 由 MCP roots 协议设置的项目根目录（最高优先级）
 * 在 MCP Server 连接后通过 listRoots() 获取 IDE 当前打开的工作区路径
 */
let rootsProjectRoot = null;
// ============ 工具函数 ============
/**
 * 获取项目根目录
 * 检测优先级（每次调用都重新检测，无缓存）：
...
function getProjectRoot() {
    // 策略 0：MCP roots 协议（最高优先级，由 IDE 自动提供）
    if (rootsProjectRoot) {
        return rootsProjectRoot;
    }
    // 策略 1：环境变量（用户手动指定，兜底方案）
    ...
    if (envRoot) {
        ...
        // 环境变量无效时不抛错，继续尝试其他策略
        console.error(`[warn] EASYAI_PROJECT_ROOT 指向的目录不包含 ${TRELLIS_ROOT}: ` +
            `${resolvedRoot}，尝试其他策略`);
    }
    ...
    throw new Error(`无法找到项目根目录：未找到 ${TRELLIS_ROOT} 目录。` +
        `已尝试：roots(未设置), env(${envRoot || "未设置"}), ` +
        `cwd(${process.cwd()}), __dirname(${__dirname})。` +
        `请确保 IDE 支持 MCP roots 协议，或设置 EASYAI_PROJECT_ROOT 环境变量。`);
}
/**
 ...
 */
function setProjectRootFromRoots(rootPath) {
    const resolvedPath = path.resolve(rootPath);
    const trellisPath = path.join(resolvedPath, TRELLIS_ROOT);
    if (fs.existsSync(trellisPath) && fs.statSync(trellisPath).isDirectory()) {
        rootsProjectRoot = resolvedPath;
        console.error(`[roots] 项目根目录已设置: ${resolvedPath}`);
    }
    else {
        console.error(`[roots] 工作区 ${resolvedPath} 不包含 ${TRELLIS_ROOT}，忽略`);
    }
}
/**
 * 重置项目根目录（清除 roots 设置）
...
/**
 * 递归扫描归档目录，提取所有匹配 T{NNN}- 格式的任务编号
 * 支持嵌套子目录结构（如 archive/YYYY-MM/T001-xxx/）
 * @param dir - 待扫描目录
 * @param callback - 每找到一个编号时回调
 */
function scanArchiveDirsRecursive(dir, callback) {
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            ...
            if (match) {
                ...
                // 非任务目录（如 YYYY-MM/），递归扫描其子目录
                scanArchiveDirsRecursive(path.join(dir, entry.name), callback);
            }
        }
    }
}
 ...
 */
function generateTaskId(title, role) {
    ...
    // 扫描归档目录中的任务（递归），防止 ID 重用
    // 归档目录支持嵌套子目录（如 archive/YYYY-MM/T001-xxx/）
    try {
        if (fs.existsSync(archiveDir)) {
            scanArchiveDirsRecursive(archiveDir, (num) => {
                if (num > maxNumber) {
                    maxNumber = num;
                }
            });
            ...
            while (i < lines.length) {
                ...
                if (lineStr === "" ||
                    ...
                    if (lineStr !== "") {
                        ...
                        blockLines.push(lineStr.substring(Math.min(leadingSpaces, 2))); // 保留最多 2 个空格的缩进
                    }
                    else {
 ...
 * @param body 任务正文
 */
function getRequiredCliCount(body) {
    const match = body.match(/- \s*CLI\s*审核数量[：:]\s*(\d+)/i);
    if (match && match[1]) {
 ...
 */
function getActualCliRecordCount(taskPath) {
    const cliDir = path.join(taskPath, "cli");
