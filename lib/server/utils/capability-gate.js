...
function normalizeRole(role) {
    // 防御性检查：非字符串输入直接返回 null（T010 修复 + 诊断增强）
    if (!role || typeof role !== "string") {
        // 诊断日志：非预期类型时输出详细信息便于排查
        if (role !== undefined && role !== null && role !== "") {
            console.error(`[capability-gate] normalizeRole 收到非字符串参数:` +
                ` typeof=${typeof role}, value=${String(role).substring(0, 50)}`);
        }
        return null;
    }
