...
/**
 * 上下文预算阈值（Token 数）
 * 来源：.trellis/config/context-budget.md + 架构 §6.3
 */
const BUDGET_THRESHOLDS = {
    WARNING: 0.6, // 60% — 自动降级 recommended → deferred
    CRITICAL: 0.8, // 80% — 建议新开会话
};
const PHASE_BUDGET = {
    plan: 40000,
    implement: 60000,
...
export function register(server) {
    server.tool("context_budget", "估算当前任务的上下文 Token 消耗。超过 60% 时建议降级 recommended 上下文，超过 80% 时建议新开会话。可选传入 task_id 分析特定任务。", {
        task_id: z.string().optional().describe("可选任务 ID，分析特定任务的上下文预算"),
        phase: z.string().optional().describe("阶段标识，决定预算上限"),
    }, async ({ task_id, phase }) => {
        try {
            const budget = PHASE_BUDGET[phase || "implement"] || DEFAULT_BUDGET;
            let totalTokens = 0;
            // 按 priority 分层统计
            const breakdown = {
