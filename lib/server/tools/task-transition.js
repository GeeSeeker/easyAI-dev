...
import { z } from "zod";
import { getTaskDir, getArchiveDir, parseTaskMd, serializeTaskMd, isValidTransition, ensureDir, VALID_TRANSITIONS, getRequiredCliCount, getActualCliRecordCount, } from "../utils/task-utils.js";
    ...
    server.tool("task_transition", "校验状态转移合法性。转移到 under_review 时启动 Evidence Gate：拒绝 _FAIL 标记，要求每个验证类别为 _PASS 或 _NA。转移到 archived 时将任务移到归档目录。", {
        ...
        try {
            ...
            if (new_status === "under_review") {
                // ABCDE CLI 记录校验
                const requiredCliCount = getRequiredCliCount(parsed.body);
                if (requiredCliCount > 0) {
                    const actualCliCount = getActualCliRecordCount(taskPath);
                    if (actualCliCount < requiredCliCount) {
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: JSON.stringify({
                                        error: true,
                                        message: `转移到 under_review 被拦截（Evidence Gate）：任务约束要求 ${requiredCliCount} 个 CLI 审查记录，当前仅有 ${actualCliCount} 个。`,
                                    }),
                                },
                            ],
                            isError: true,
