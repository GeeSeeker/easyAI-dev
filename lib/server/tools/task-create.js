...
import { z } from "zod";
import { generateTaskId, getTaskDir, serializeTaskMd, ensureDir, } from "../utils/task-utils.js";
...
export function register(server) {
    server.tool("task_create", "创建新任务。生成任务 ID，创建任务目录和 task.md 文件，初始状态为 pending。", {
        title: z.string(),
    ...
    }, async ({ title, description, acceptance_criteria, file_scope, assign_strategy, role, }) => {
        try {
            // Capability Gate: task_create 仅 PM 可调用
            const reject = checkCapability(role, "task_create");
            if (reject) {
