...
const FRAMEWORK_VERSION = "4.3.30";
// Manifest 相关常量
...
// ============ Config 差异检测 ============
/**
 * 从 YAML 内容提取带路径的 key 列表
 * 通过缩进层级构建 parent.child 路径，精确区分同名 key
 */
function extractYamlPaths(content) {
    const paths = new Set();
    const stack = [];
...
/**
 * 写入 config 差异文件，供 pm-session-start AI 引导消费
 * @returns 缺失路径数量（0 表示无差异）
 */
function writeConfigPending(targetDir, userSnapshot, skeletonConfigPath, frameworkVersion) {
    try {
        if (!fs.existsSync(skeletonConfigPath))
            return 0;
        const skeletonContent = fs.readFileSync(skeletonConfigPath, "utf-8");
        const userPaths = extractYamlPaths(userSnapshot);
        const skeletonPaths = extractYamlPaths(skeletonContent);
        const allMissing = [];
        ...
        // 只保留叶子节点
        const missingPaths = allMissing.filter((p) => {
            return !allMissing.some((other) => other !== p && other.startsWith(p + "."));
        });
        if (missingPaths.length === 0)
            return 0;
        const pendingPath = path.join(targetDir, ".trellis", "config", ".config-pending.json");
        const pending = {
            fromVersion: userSnapshot.match(/version:\s*"([^"]+)"/)?.[1] || "unknown",
            toVersion: frameworkVersion,
            missingPaths,
            skeletonSnapshot: skeletonContent,
...
export function registerFrameworkInit(server) {
    server.tool("framework_init", "在指定目录初始化 easyAI 框架。复制 .agents/、.trellis/、.docs/ 骨架到项目中。" +
        ...
        try {
            const targetDir = inputDir || getProjectRoot();
            ...
            if (fs.existsSync(trellisSrc)) {
                ...
                // 清除硬编码的 developer.name，以保证 onboarding 能够正常触发
                // 注意：精准匹配 developer.name，保留 framework, team, external_cli 等其他预填字段
                const configPath = path.join(resolvedDir, ".trellis/config/config.yaml");
                if (fs.existsSync(configPath)) {
                    let configContent = fs.readFileSync(configPath, "utf-8");
                    // 仅匹配 developer 块下的 name: "xxxx" 并替换为空字符串
                    configContent = configContent.replace(/(developer:[\s\n]+name:\s*")[^"]*(")/, '$1$2');
                    fs.writeFileSync(configPath, configContent);
                    logs.push("✓ config.yaml — 已重置 developer.name 等待引导");
                }
            }
            // 复制 .docs/
...
export function registerFrameworkCheck(server) {
    server.tool("framework_check", "检查当前项目的 easyAI 框架完整性。验证必需目录、文件、版本和 Git 状态。", {
        ...
        try {
            ...
            const resolvedDir = path.resolve(targetDir);
            // 查询 npm 注册表获取真正的最新版本
            let npmLatest = FRAMEWORK_VERSION;
            try {
                const npmResult = execSync("npm view @geeseeker/easyai-dev version", {
                    timeout: 10000,
                    stdio: "pipe",
                });
                const parsed = npmResult.toString().trim();
                if (parsed)
                    npmLatest = parsed;
            }
            catch {
            ...
            // 取 npm 最新版和 MCP Server 版本中较高的（防止 npm 缓存滞后）
            const latestVersion = compareSemver(npmLatest, FRAMEWORK_VERSION) > 0
                ? npmLatest
                : FRAMEWORK_VERSION;
            const results = {
                dirs: [],
            ...
            if (fs.existsSync(versionFile)) {
                ...
                results.version.upToDate = installed === latestVersion;
...
export function registerFrameworkUpdate(server) {
    server.tool("framework_update", "更新 easyAI 框架到最新版本。使用 Manifest 驱动的智能合并：" +
        ...
        try {
            ...
            const BACKUP_DIR = path.join(resolvedDir, ".tmp", "framework-upgrade-backup");
            const STAGING_DIR = path.join(resolvedDir, ".tmp", "framework-upgrade-staging");
            ...
            for (const [relPath, newEntry] of Object.entries(newManifest.files)) {
                ...
                // merge_mode: append → 智能追加新顶层段，并写差异文件
                if (newEntry.merge_mode === "append" && fs.existsSync(targetPath)) {
                    // 保存用户原始快照（appendMissingSections 会修改 staging 文件）
                    const userSnapshot = fs.readFileSync(targetPath, "utf-8");
                    const backupPath = path.join(BACKUP_DIR, relPath);
                    fs.mkdirSync(path.dirname(backupPath), { recursive: true });
                    fs.copyFileSync(targetPath, backupPath);
                    const stagingPath = path.join(STAGING_DIR, relPath);
                    fs.mkdirSync(path.dirname(stagingPath), { recursive: true });
                    fs.copyFileSync(targetPath, stagingPath);
                    const appendLogs = appendMissingSections(stagingPath, skeletonPath, FRAMEWORK_VERSION);
                    logs.push(...appendLogs);
                    // 写差异文件到项目目录（不经 staging，直接写入）
                    const pendingCount = writeConfigPending(resolvedDir, userSnapshot, skeletonPath, FRAMEWORK_VERSION);
                    if (pendingCount > 0) {
                        logs.push(`💡 检测到 ${pendingCount} 个新配置项，` +
                            `PM 会话启动时将协助配置`);
                    }
                    continue;
                }
                if (!fs.existsSync(targetPath)) {
                    const stagingPath = path.join(STAGING_DIR, relPath);
                    fs.mkdirSync(path.dirname(stagingPath), { recursive: true });
                    fs.copyFileSync(skeletonPath, stagingPath);
                    logs.push(`+ ${relPath} — 新增`);
                    created++;
                    continue;
                }
                if (!oldEntry) {
                    const stagingPath = path.join(STAGING_DIR, relPath + ".new");
                    fs.mkdirSync(path.dirname(stagingPath), { recursive: true });
                    fs.copyFileSync(skeletonPath, stagingPath);
                    conflicts.push({
                        file: relPath,
            ...
            // 更新 Manifest 和版本号放入 staging
            const oldManifestBackupPath = path.join(BACKUP_DIR, `.${MANIFEST_FILENAME}`);
            if (fs.existsSync(oldManifestPath)) {
                fs.copyFileSync(oldManifestPath, oldManifestBackupPath);
            }
            const versionBackupPath = path.join(BACKUP_DIR, ".easyai-version");
            if (fs.existsSync(versionFile)) {
                fs.copyFileSync(versionFile, versionBackupPath);
            }
            const stagingOldManifestPath = path.join(STAGING_DIR, `.${MANIFEST_FILENAME}`);
            fs.copyFileSync(newManifestPath, stagingOldManifestPath);
            const stagingVersionFile = path.join(STAGING_DIR, ".easyai-version");
            fs.writeFileSync(stagingVersionFile, `${FRAMEWORK_VERSION}\n`);
            // Phase 2: 原子移动与删除
            try {
                const applyDir = (src, dest) => {
                    const entries = fs.readdirSync(src, { withFileTypes: true });
                    for (const entry of entries) {
                        const srcPath = path.join(src, entry.name);
                        const destPath = path.join(dest, entry.name);
                ...
                applyDir(STAGING_DIR, resolvedDir);
                ...
                applyDirCopy(BACKUP_DIR, resolvedDir);
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({
                                success: false,
                                error: `更新过程失败，已触发回滚。备份保留在 ${BACKUP_DIR}。错误详细信息: ${applyErr instanceof Error ? applyErr.message : String(applyErr)}`,
