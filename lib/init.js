...
export async function doUpdate(targetDir) {
  ...
  // config.yaml 新字段检测提示
  detectNewConfigFields(targetDir);
 ...
 * 对比用户文件和骨架文件中的所有"key: value"行，
 * 找出骨架中有但用户文件中没有的 key
 * @param {string} targetDir - 用户项目目录
 */
function detectNewConfigFields(targetDir) {
  try {
    ...
    if (!fs.existsSync(userConfigPath) || !fs.existsSync(skeletonConfigPath)) {
      return;
    }
    ...
    // 提取所有配置 key（含缩进层级），排除注释和空行
    const extractKeys = (content) => {
      const keys = new Set();
      for (const line of content.split("\n")) {
        // 匹配 "  key:" 或 "key:" 格式（排除注释、列表项、纯值行）
        const m = line.match(/^(\s*)([a-z_][a-z0-9_]*):\s*/);
        if (m && !line.trim().startsWith("#")) {
          // 用缩进 + key 名作为唯一标识，区分不同层级
          keys.add(`${m[1].length}:${m[2]}`);
        }
      }
      return keys;
    };

    const userKeys = extractKeys(userContent);
    const skeletonKeys = extractKeys(skeletonContent);

    // 找出骨架有但用户没有的 key
    const missingKeys = [];
    for (const key of skeletonKeys) {
      if (!userKeys.has(key)) {
        missingKeys.push(key.split(":")[1]); // 只取 key 名
      }
    }
    ...
    if (missingKeys.length > 0) {
      console.log(
        `\n${YELLOW}${BOLD}💡 config.yaml 发现 ` +
          `${missingKeys.length} 个新配置项:${NC}` +
          ` ${missingKeys.join(", ")}`,
      );
      console.log(
        `${YELLOW}   建议对比骨架最新配置，手动添加到你的 config.yaml 中${NC}`,
      );
      console.log(
        `${YELLOW}   骨架路径: ` +
          `npx @geeseeker/easyai-dev init --show-config${NC}`,
      );
    }
  } catch (_err) {
    // 检测失败不阻塞 update 流程
