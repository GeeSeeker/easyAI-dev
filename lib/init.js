...
export async function doUpdate(targetDir) {
  ...
  // config.yaml 新字段检测 → 写差异文件供 AI 引导
  // 使用升级前保存的快照（而非被 appendMissingSections 修改后的文件）
  writeConfigPending(targetDir, configSnapshot);
...
/**
 * 检测 config.yaml 中用户缺少的新配置项，写入差异文件
 * 差异文件由 pm-session-start 的 AI 引导流程消费
 * @param {string} targetDir - 用户项目目录
 * @param {string|null} userSnapshot - 升级前用户 config 内容快照
 */
function writeConfigPending(targetDir, userSnapshot) {
  try {
    ...
    const pendingPath = path.join(
      targetDir, ".trellis", "config", ".config-pending.json",
    );
    const pending = {
    ...
    console.log(
      `\n${YELLOW}${BOLD}💡 检测到 ${missingPaths.length} 个新配置项，` +
        `下次启动 PM 会话时 AI 将协助你配置${NC}`,
    );
  } catch (err) {
    // 检测失败不阻塞 update 流程
    console.error(`⚠️ config 差异检测失败: ${err.message}`);
  }
}
