...
export async function doInit(targetDir) {
  ...
  console.log(`
${BOLD}下一步：配置 MCP Server${NC}（一次性，所有项目共用）
...
配置完成后，重启 IDE，输入 ${GREEN}/actor-pm${NC} 即可启动项目经理开始工作。
${YELLOW}提示${NC}：MCP Server 会自动识别当前打开的项目，无需手动指定路径。
`);
}
...
export async function doUpdate(targetDir) {
  ...
  // 保存用户 config.yaml 快照（在 appendMissingSections 修改前）
  // 供后续 detectNewConfigFields 对比使用
  let configSnapshot = null;
  const configPath = path.join(
    targetDir, ".trellis", "config", "config.yaml",
  );
  ...
  // config.yaml 新字段检测提示
  // 使用升级前保存的快照（而非被 appendMissingSections 修改后的文件）
  detectNewConfigFields(targetDir, configSnapshot);
 ...
 * @returns {Set<string>} key 路径集合 如 {"framework", "framework.version", "team.roster.timeout"}
 */
...
/**
 * 检测 config.yaml 中用户缺少的配置项
 * 对比用户原始文件（升级前快照）和骨架文件的 key 路径差异
 * @param {string} targetDir - 用户项目目录
 * @param {string|null} userSnapshot - 升级前用户 config 内容快照
 */
function detectNewConfigFields(targetDir, userSnapshot) {
  try {
    const skeletonConfigPath = path.join(
      SKELETON_DIR, ".trellis", "config", "config.yaml",
    );

    if (!userSnapshot || !fs.existsSync(skeletonConfigPath)) {
      return;
    }
    ...
    // 过滤：只显示叶子级别有意义的新字段（排除纯结构性父节点）
    // 如果某个路径的所有子路径也是 missing，只显示父路径即可
    const meaningful = missingPaths.filter((p) => {
      // 如果它是某个 missing 路径的父节点，跳过（会被子路径覆盖说明）
      const isParent = missingPaths.some(
    ...
    if (meaningful.length > 0) {
      // 提取最后一段 key 名用于显示
      const keyNames = meaningful.map((p) => p.split(".").pop());
      // 去重
      const unique = [...new Set(keyNames)];
      console.log(
        `\n${YELLOW}${BOLD}💡 config.yaml 发现 ` +
          `${unique.length} 个新配置项:${NC}` +
          ` ${unique.join(", ")}`,
      );
      console.log(
        `${YELLOW}   建议对比最新骨架配置，手动添加到你的 config.yaml 中${NC}`,
      );
      console.log(
        `${YELLOW}   可运行: npx @geeseeker/easyai-dev check .` +
          ` 查看完整性${NC}`,
      );
    }
  } catch (_err) {
