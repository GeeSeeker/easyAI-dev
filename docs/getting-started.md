...
> MCP Server 会**自动检测** IDE 当前打开的项目（通过 MCP roots 协议），无需手动配置路径。
...
**首次启动？** PM 会检测到这是新项目，自动进入**引导教程**：

1. 引导你填写开发者信息、编码规范、AI 审查团队配置
2. 带你体验记事本、任务创建、Worker 执行等核心流程
3. 每一步都有讲解，帮你理解 easyAI 的工作方式

> 熟手可以选择「快速模式」，AI 自动完成所有初始化并输出功能速查表。

引导完成后（或非首次启动的正常会话），试着告诉 PM 你想做什么。比如：
...
工作完成后，**一定要告诉 PM「收工」**。这是 easyAI 跨会话记忆的关键：
...
1. **写入 journal 日志** — 记录本次会话做了什么、发现了什么、遗留了什么
2. **Git 自动提交** — 保存所有文件变更
3. **输出恢复指引** — 下次启动 PM 时，AI 会自动读取 journal 恢复上下文

> ⚠️ **不收工 = 下次 AI 可能丢失本次工作上下文。** 就像你写了一天代码但没保存。

---

## 9. ⭐ 进度归档：里程碑收纳

当你完成一个**阶段性目标**（比如一个版本发布、一个大功能上线）时，告诉 PM：
...
1. **文档清理** — 过期草稿标记归档、已完成设计文档打上 `[DONE]` 标记
2. **任务归档** — 已完成任务移入 `archive/`
3. **临时文件清理** — 清空临时缓存
4. **里程碑总结** — 生成阶段性成果报告，沉淀到 journal

> 收工 vs 归档：**收工**是每次结束工作时做的（像下班），**归档**是完成一个里程碑时做的（像季度总结）。

---

## 10. 下一步

恭喜！你已经了解了 easyAI 的完整工作流程。接下来：
