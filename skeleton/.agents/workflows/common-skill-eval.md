...
### Step 3：自评 ABCDE 工作重要性等级（强制风险评估）

无论当前角色是 PM 还是 Worker，在产出离散产出物时，**必须**完成以下自评流程。

#### 3a. 风险因素评估（不可省略）

逐项检查以下 5 个风险因素，回答"是/否"：

| #   | 风险因素                                           | 命中等级 |
| --- | -------------------------------------------------- | -------- |
| R1  | 是否修改 `.agents/`、`.trellis/spec/` 等框架核心？ | → A      |
| R2  | 是否涉及 MCP Server（`packages/`）或状态机变更？   | → A      |
| R3  | 是否跨 2+ 个模块且用户可感知？                     | → B      |
| R4  | 是否仅影响单一模块的内部逻辑？                     | → C      |
| R5  | 是否纯文档/注释/配置/测试且无功能变更？            | → D/E    |

**定级规则**：取命中的**最高等级**（A > B > C > D > E）。若无命中项，默认 D。

#### 3b. ABCDE 等级定义与 CLI 审核要求
...
若当前任务的 `task.md` 中已有 PM 的**路由配置**（`ABCDE 等级` / `CLI 审核数量`），执行交叉验证：

- **自评 = PM 路由** → 正常继续
- **自评 < PM 路由**（如自评 C，PM 标 B）→ **从严执行**，按 PM 路由等级的 CLI 审核数量操作
- **自评 > PM 路由**（如自评 A，PM 标 C）→ **上报 PM**，在执行记录中写入 `[GRADE_ESCALATION: 自评 A > 路由 C]`，等待 PM 确认
...
```markdown
> **📊 工作分级：{等级} 级** — {定义}，需 {n} 个 CLI 审核
> 风险因素：R{x}={是/否}, R{y}={是/否}, ...
> 路由验证：{一致 / 从严 / 已上报}
```

- A/B 级**必须**在日志中（`task_append_log`）记录分级理由
...
| `worker-five-dimension-audit` | PM 创建的专项审计任务（约束集含 `[AUDIT]` 标记） | 日常功能开发、Bug 修复、文档编辑 |
