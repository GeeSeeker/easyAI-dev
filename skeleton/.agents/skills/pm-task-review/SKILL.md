...
根据 `file_scope` 和任务性质确定风险等级（🟢低 / 🟡中 / 🔴高 / ⚫极高）。低风险任务 Stage 2 可简化为快速检查。
...
- 🟢 **低风险**：只查 Critical 级问题，跳过代码风格/命名细节
- 🔴 **高风险**：PM **必须**独立验证关键路径（运行测试或手动验证）
- ⚫ **极高风险**：PM 独立验证 + **必须**至少 1 个外部 CLI 独立审查
...
3. **外部 CLI 独立审查**
   - ⚫ **极高风险**：**必须**至少调用 1 个外部 CLI（Codex / Claude Code）做独立审查
   - 其他风险等级：可选，检测复杂任务时建议使用
   - 外部 CLI 产出写入 `cli/` 目录
   - 综合各方意见形成最终判断
