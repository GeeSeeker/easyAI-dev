...
- 🟢 **D 级**（PM 自审）：PM 自审，只查 Critical 级问题，跳过代码风格/命名细节
- 🟡 **C 级**（1 CLI）：PM 审查 + **必须** 1 个外部 CLI 独立审查
- 🔴 **B 级**（2 CLI）：PM 审查 + **必须** 2 个外部 CLI 并行独立审查
- ⚫ **A 级**（3 CLI）：PM 独立验证关键路径 + **必须** 3 个外部 CLI 并行独立审查
