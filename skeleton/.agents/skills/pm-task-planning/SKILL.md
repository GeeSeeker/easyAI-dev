---
name: pm-task-planning
description: "[PM] 任务规划 — 触发：设计文档已确认，需创建执行任务。排除：设计未持久化到 .docs/design/。产出：约束集任务（task_create）。"
produces: task_with_constraints
requires: design_document
---

# PM 任务规划（约束集驱动）

## 适用场景

PM 完成 `pm-brainstorm`（需求澄清 + 设计确认）后，**必须**激活本 Skill 将设计转化为可执行的约束集任务。

## 核心理念

> 来源：ccg-workflow spec-research / spec-plan

- PM 产出的**不是** TODO List，而是**约束集**
- 每条约束缩小执行者的解空间，使执行者可"零决策"机械执行
- 所有技术决策**必须**在规划阶段完成，禁止推迟到实现阶段
- 约束集必须精准到可验证：含具体参数、明确边界、可测量标准

## HARD-GATE

> 补充检查点详见 `spec://guides/review-standards` §五

```text
1. 设计文档未持久化到 .docs/design/ 前，不得进入本流程。
2. plan_validate() 检测到反面模式时，必须修正后才能创建任务。
3. conflict_check() 检测到文件冲突时，必须解决后才能创建任务。
4. context.jsonl 文件缺失时，不得向用户发出执行指引（允许显式空清单 `[]`，不允许文件不存在）。
5. 验收标准中存在不可 yes/no 判定的模糊条件时，必须重写为可测标准。
```

## 流程

### Step 1：需求聚焦

- 读取已确认的设计文档（来自 `pm-brainstorm` 的 `.docs/design/{name}.md`）
- 提取功能边界、技术约束、验收标准草案
- 读取 `.trellis/spec/` 中与任务相关的规范文件，确保约束集不与现有规范冲突
- 确认设计文档路径，后续 `task_create()` 将引用此路径

### Step 2：约束集生成

将需求转化为约束集格式。约束集分三层：

#### 硬约束（不可违反）

技术选型、接口规范、安全要求等确定性决策——执行者必须遵守，无商量余地。

示例：

- C1: 使用 JWT，TTL=15min，刷新机制=sliding window
- C2: API 端点遵循 RESTful 规范，路径前缀 `/api/v1/`
- C3: 密码存储使用 bcrypt，cost=12

#### 软约束（建议遵守）

代码风格偏好、辅助性建议——执行者有小幅调整空间，偏离时需说明理由。

示例：

- C4: 优先复用 `src/utils/` 中的现有函数
- C5: 单个函数不超过 30 行

#### 验收标准

可验证的成功判据——每条必须是 yes/no 可判定的。

示例：

- [ ] 登录成功返回有效 JWT token
- [ ] 无效凭证返回 401 + 标准错误格式
- [ ] token 过期后请求返回 401

### Step 3：反面模式自检（HARD-GATE #2）

**必须**在约束集完成后调用 `plan_validate()` 检测以下反面模式：

1. **多方案未选择** — 列出方案但未决定 → 必须选定一个并说明理由
2. **无决策标准** — 使用"都可以""均可"等模糊词 → 必须给出选择依据
3. **推迟决策** — 标记"待定""TBD" → 所有决策在此阶段完成
4. **信息堆砌无结论** — 列举信息无总结 → 以约束或决策收尾

```
调用方式：plan_validate(content: "约束集完整内容")
```

- `valid: true` → 继续 Step 4
- `valid: false` → 修正检测到的反面模式，重新调用直到通过

### Step 4：任务拆分

按约束集拆分为**原子任务**，每个任务满足：

- **单一职责**：一个任务做一件事
- **可独立验收**：有明确的完成判据
- **文件范围明确**：列出所有涉及文件/目录
- **依赖显式化**：如有前置依赖，声明依赖任务 ID
- **粒度适中**：每个任务执行者在 1-2 个会话内可完成

每个任务包含：

```
标题：简洁描述
约束集：从 Step 2 筛选该任务相关约束
文件范围：涉及的文件/目录 glob 列表
验收标准：从 Step 2 筛选该任务相关标准
设计文档：引用 .docs/design/{name}.md 路径
```

### Step 4.5：任务路由决策（F39 + O3 + F43）

为每个任务确定**执行模式**和**审查策略**。读取 `.trellis/config/config.yaml` 中的 `routing` 和 `team` 配置。

#### 1. 风险评估

按以下优先级判定风险等级（**高风险规则优先于白名单**）：

1. **强制高风险检查**（来自 `spec://guides/review-standards`）：
   - `.agents/` 下 Skill/Workflow 变更 → 🔴 高风险（无论是否匹配白名单）
   - 修改公共接口 / 涉及安全逻辑 / 跨模块依赖 → 🔴 高风险
   - 架构变更 / 数据迁移 / 破坏性变更 → ⚫ 极高风险
2. **白名单匹配**：上一步未命中高风险，且任务 `file_scope` 中**所有文件**均属于 `routing.low_risk_whitelist` 中的变更类型 → 🟢 低风险
   - 混合类型（白名单 + 非白名单）→ 跳过白名单，进入影响面判断
3. **影响面判断**：以上均不适用时：
   - 仅影响 1-2 个文件，无跨模块依赖 → 🟡 中风险
   - 其他 → 🔴 高风险

#### 2. 审查策略选择

根据 `routing.risk_review_mapping` 映射（key 使用 `low/medium/high/critical` 对应 🟢/🟡/🔴/⚫）：

| 风险等级 | config key | 策略                     | 说明                               |
| -------- | ---------- | ------------------------ | ---------------------------------- |
| 🟢 低    | `low`      | `pm_self_review`         | PM 自审，跳过代码风格细节          |
| 🟡 中    | `medium`   | `single_cli_review`      | 1 个外部 CLI 审查                  |
| 🔴 高    | `high`     | `dual_cli_review`        | 2 个外部 CLI 并行审查              |
| ⚫ 极高  | `critical` | `dual_cli_plus_internal` | 2 个外部 CLI + PM 独立验证关键路径 |

#### 3. 审查者匹配

从 `team.roster`（`type: external_cli` 的条目）中，根据任务领域匹配 `strengths`：

| 任务领域             | 首选 CLI    | 备选 CLI    | 匹配依据                            |
| -------------------- | ----------- | ----------- | ----------------------------------- |
| 后端逻辑 / 安全      | Codex CLI   | Claude Code | strengths: 后端逻辑, 安全检查       |
| 架构 / 重构 / 多文件 | Claude Code | Codex CLI   | strengths: 架构分析, 多文件关联分析 |
| 前端 / 模式一致性    | Gemini CLI  | Claude Code | strengths: 前端审查, 模式一致性检查 |

- 🟡 中风险（1 CLI）→ 选首选 CLI
- 🔴/⚫（2 CLI）→ 首选 + 备选组合（确保差异化视角）

> **注**：`concurrency` 字段为保留字段，当前所有 CLI 并发配额为 1（单实例）。未来支持多实例时，PM 在此步骤检查当前占用情况，超额时降级到单 CLI 审查。

#### 4. 执行模式选择

根据任务复杂度决定执行模式：

- **独立执行者**（默认）：任务预估 1-2 个会话可完成
- **组长+组员**：任务预估超过 2 个会话，或包含可并行的子任务
- ⚫ 极高风险任务 → 建议组长+组员（多人交叉验证）

> PM 可根据实际情况调整推荐，路由配置为建议而非强制。

### Step 4.6：规划产出外部 CLI 审核

> 风险等级 🟡 以上的任务，实施计划必须经外部 CLI 审核后再呈现用户。

1. **检查风险等级**：Step 4.5 判定为 🟡 中、🔴 高或 ⚫ 极高时触发本步骤；🟢 低风险跳过
2. **准备审核材料**：将约束集 + 验收标准 + 文件范围整理为结构化摘要
3. **调用外部 CLI 审核**：
   - 🟡 中风险 → 调用 1 个 CLI（首选匹配）
   - 🔴 高 / ⚫ 极高 → 调用 2 个 CLI 并行审核
4. **审核关注点**：
   - 约束集是否有遗漏或冲突
   - 验收标准是否可测量、无歧义
   - 文件范围是否完整（有无遗漏的关联文件）
   - 任务粒度是否合理
5. **处理审核结果**：
   - 无修改意见 → 继续 Step 5
   - 有修改意见 → 修正约束集后重审（最多 2 轮）
   - 2 轮后仍有争议 → 记录分歧，连同 PM 判断一起呈现用户决策

#### 5. 路由结果写入

将路由决策追加到 `task_create()` 的 `description` 中：

```markdown
## 路由配置

- 风险等级：{low/medium/high/critical}（{🟢/🟡/🔴/⚫}）
- 审查策略：{pm_self_review/single_cli_review/dual_cli_review/dual_cli_plus_internal}
- 推荐审查者：{CLI 名称列表}
- 执行模式：{独立执行者/组长+组员}
```

### Step 5：冲突预检（HARD-GATE #3）

**必须**在创建任务前调用 `conflict_check()` 检测文件范围重叠：

```
调用方式：conflict_check(files: ["src/auth/*.ts", ...], role: "pm")
```

- 无冲突 → 继续 Step 6
- 有冲突 → 向用户呈现冲突报告，解决方案：
  - 调整文件范围避开冲突
  - 建立任务依赖（先完成冲突任务）
  - 使用 `worktree_create()` 物理隔离

### Step 6：任务创建

调用 `task_create()` 创建任务：

```text
task_create(
  role: "pm",
  title: "实现用户认证 API",
  description: "参见设计文档：.docs/design/user-auth.md\n\n## 约束集\n...",
  acceptance_criteria: "1. 登录返回 JWT\n2. ...",
  file_scope: "src/auth/*.ts, src/middleware/jwt.ts",
  assign_strategy: "independent"
)
```

### Step 7：上下文配置

任务创建成功后，为每个任务调用 `context_generate()` 生成 context.jsonl 推荐清单：

```text
调用方式：context_generate(task_id: "T001", phase: "implement")
```

审核推荐清单：

1. 添加遗漏的必要规范（特别是 Step 1 中识别的相关 spec 路径）
2. 移除不相关的推荐
3. **框架任务时，将相关图谱节点路径写入 context.jsonl**：

   ```json
   {
     "uri": ".agents/graph/xxx.md",
     "priority": "recommended",
     "reason": "涉及该特性的上下游依赖"
   }
   ```

   - 框架任务 → 必须写入图谱节点
   - 业务任务涉及框架结构修改 → 建议写入
   - 纯业务逻辑任务 → 不需要

4. **将审核后的清单写入 `.trellis/tasks/{id}/context.jsonl`**（HARD-GATE）

> ⚠️ 不写入 context.jsonl = Worker 拿不到 spec 清单 = 违规。

创建完成后向用户汇报：

- 任务 ID 和标题
- 执行指引（如"请开新会话执行 `/actor-worker T001`"）

## 约束集输出模板

```markdown
## 约束集

### 硬约束（不可违反）

- C1: [具体技术决策 + 参数]
- C2: [接口规范 + 格式要求]

### 软约束（建议遵守）

- C3: [代码风格偏好]
- C4: [辅助性建议]

### 文件范围

- path/to/dir/\*.ext
- path/to/specific-file.ext

### 验收标准

- [ ] [可验证的成功判据 1]
- [ ] [可验证的成功判据 2]

### 设计文档

- .docs/design/{feature-name}.md
```

## 关键原则

- **缩小解空间** — 每条约束都让执行者少做一个决策
- **具体可验证** — "使用 bcrypt cost=12"而非"使用安全的密码存储"
- **YAGNI 无情** — 移除所有"以后可能用到"的约束
- **原子化拆分** — 任务粒度以 1-2 个执行者会话为标准

## PATEOAS 导航

```markdown
### 下一步行动

1. [ ] 读取设计文档，提取功能边界和技术约束
2. [ ] 生成约束集（硬约束 + 软约束 + 验收标准）
3. [ ] 调用 `plan_validate()` 自检反面模式
4. [ ] 拆分为原子任务（单一职责 + 文件范围 + 验收标准）
5. [ ] 任务路由决策（风险评估 + 审查策略 + 审查者匹配）
6. [ ] 调用 `conflict_check()` 检测文件冲突
7. [ ] 调用 `task_create()` 创建任务（含路由配置段）
8. [ ] 调用 `context_generate()` 生成推荐清单，审核后写入 `context.jsonl`

### 状态快照

- 当前角色：PM
- 当前阶段：TASK_PLANNING
- 设计文档：{已读取/未读取}
- 约束集：{未开始/生成中/已完成}
- 反面模式自检：{未开始/通过/未通过}
- 路由决策：{未开始/已完成}
- 冲突检测：{未开始/通过/有冲突}
- 任务创建：{未开始/已创建 x 个}
```

## 触发测试用例

### TC-01: 设计文档存在时正常规划

- **场景**: PM 完成 brainstorm 后激活本 Skill
- **输入**: 已有 `.docs/design/user-auth.md` 设计文档
- **期望行为**:
  1. Step 1 读取设计文档
  2. Step 2 生成约束集（硬约束 + 软约束 + 验收标准）
  3. Step 3 调用 `plan_validate()` 自检
  4. Step 4 拆分原子任务
  5. Step 4.5 路由决策
  6. Step 5 冲突预检
  7. Step 6 调用 `task_create()` 创建任务
  8. Step 7 生成 context.jsonl
- **验证方法**: 检查任务是否包含约束集、路由配置、context.jsonl

### TC-02: 高风险任务触发 Step 4.6 CLI 审核

- **场景**: 任务涉及 `.agents/` 下 Skill 变更
- **输入**: file_scope 包含 `.agents/skills/*/SKILL.md`
- **期望行为**: Step 4.5 判定 🔴 高风险 → 触发 Step 4.6 外部 CLI 审核 → 2 个 CLI 并行审核约束集
- **验证方法**: 检查是否在 Step 4.6 调用了外部 CLI，且审核通过后才进入 Step 5
