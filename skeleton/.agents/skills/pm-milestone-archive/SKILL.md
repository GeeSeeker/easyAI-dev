---
name: pm-milestone-archive
description: "[PM] 进度归档 — 触发：用户主动要求「进度归档」或里程碑结束。排除：日常单次会话收工（用 pm-session-close）。产出：废弃文档隔离、冷藏区转移、全局环境清理与里程碑总结报告。"
---

# `pm-milestone-archive` Skill

## 目标

这是一个独立于日常收工的重度清理与总结工具。仅在大型阶段（如 Epic 完工、版本发布）结束时，由用户输入"进度归档"主动触发。核心思想是净化热工作区视野，将已作废的意图与部件送入冷宫封存，同时生成阶段性的里程碑总结。

## 前置要求

- 当前角色必须是 **PM**。Worker 角色不执行全局归档。
- 只有明确触发"进度归档"、"里程碑结项"等宏观动作时才激活。

## 执行流程（强制分阶段）

> **核心原则：先扫描 → 再汇报方案 → 等用户确认 → 批量执行。严禁未经确认直接执行归档动作。**

### Phase A: 全局扫描

对以下所有区域进行只读扫描，收集归档候选项：

1. `.docs/`（根目录）— 扫描常青文档，检测疑似旧版本（文件名含版本号、或修改时间远早于当前阶段），发现候选项时**询问用户**是否移入 `.docs/archive/`
2. `.docs/design/` — `features/` + `planning/`
3. `.docs/refs/`
4. `.docs/notes/`（含记事本内容分析）
5. `.agents/workflows/` 和 `.agents/skills/`（是否有废弃项）
6. `.tmp/` 和 `.test/`
7. `.trellis/tasks/`（各工单状态）

### Phase B: 汇报归档方案

将扫描结果整理为分类表格，向用户列出每个文件/目录的**建议处理方式**。对于不确定的项，明确标注「需用户决策」。等待用户逐条确认或批量确认后，才进入 Phase C。

### Phase C: 批量执行

按用户确认后的方案，执行以下 5 个步骤：

---

### Step 1: `.docs/` 意图空间清理

- **`.docs/`（根目录常青文档）**：
  - 存放**项目核心常青文档**（需求文档、架构设计、SRS 等）
  - 一般由用户放入，AI 产出的符合条件的文档应主动询问用户是否存放
  - **归档时一般不动** — 属于项目级常青文档，不打 `[DONE]` 标记
  - 疑似旧版本（文件名含版本号、修改时间远早于当前阶段）→ 询问用户是否移入 `.docs/archive/`
- **`design/features/`**：
  - 完工设计打 `[DONE]` 标记，保留原位。
  - 废弃方案打 `[OBSOLETE]` 标记，移至 `design/features/archive/`。
  - **设计文档归档模板**：当设计文档被标记 `[OBSOLETE]` 并移入 `archive/` 时，在文件头部插入归档元信息块：

    ```markdown
    > [!NOTE]
    > **归档信息**
    > - 归档时间：YYYY-MM-DD
    > - 归档原因：{废弃 / 被替代 / 需求变更}
    > - 替代文档：{新文档路径，如有；无则填「无」}

    ---

    {原文档内容保持不变}
    ```

- **`design/planning/`**：
  - 已完成的设计草案打 `[DONE]` 标记，保留原位。
- **`refs/`**：
  - 仅移动明确废弃/失效的参考资料到 `.docs/refs/archive/`。
  - 仍有查阅价值的旧参考资料添加 `[DONE]` 标记，留在原地。
- **`notes/`（记事本归档 — 特殊规则）**：
  - **两个记事本的归档处理**：读取 `user-记事本.md` 和 `pm-记事本.md`，将「完成」区的所有条目**剪切**出来，写入归档文件。归档文件命名：`notes/archive/YYYY-MM-阶段名称-记事本归档.md`。原文件保留，「完成」区清空。
  - **记事本归档模板**：

    ```markdown
    # 记事本归档 — {阶段名称}

    > 归档时间：YYYY-MM-DD

    ---

    ## user-记事本

    > 来源：user-记事本.md

    {从 user-记事本.md「完成」区剪切过来的条目，保持原始格式}

    ## pm-记事本

    > 来源：pm-记事本.md

    {从 pm-记事本.md「完成」区剪切过来的条目，保持原始格式}
    ```

    - 如果只有一个记事本有内容，省略另一个记事本的二级标题段落
    - 如果两个记事本都有内容，合并到同一个归档文件中，用二级标题区分来源
  - **user-记事本的处理模式**：归档时忽略说明区中的处理模式设定（归档是用户主动触发的重度操作，不受日常处理模式限制）。
  - 清理挂着 `pm-xxx` 前缀的已转正/废弃临时草稿文件：归档到 `.docs/notes/archive/` 或提示用户删除。

### Step 2: `.archive/` 全局冷藏区清理

- 扫描是否存在已确认废弃或被新版本替代的 `.agents/workflows/` 或 `.agents/skills/`（如空目录、被替代的旧实现），向用户确认后，统一物理转移至项目根目录的冷藏区：
  - 过期工作流移入：`.archive/dep-workflows/`
  - 过期技能移入：`.archive/dep-skills/`

### Step 3: `.tmp/` 与 `.test/` 清理

- **`.tmp/`**：执行强制**全部清空**（保留 `.gitkeep` 除外）。这是纯粹的瞬态废件池。
- **`.test/`**：非框架强制目录。如存在且有内容，**提醒用户自行清理**，AI 不主动操作。

### Step 4: `.trellis/` 契约与记忆空间清理

- **任务归档**：将所有状态为 `completed` 或 `cancelled` 且非活跃的工单目录，批量移动到 `.trellis/tasks/archive/YYYY-MM/`。

  ```bash
  mkdir -p .trellis/tasks/archive/YYYY-MM
  # 对每个 completed/cancelled 工单执行 mv
  ```

### Step 5: 撰写并放置里程碑总结报告

> **里程碑 = 项目的简化版知识图谱**。图谱帮助框架理解自身架构，里程碑帮助 AI 快速追溯每个阶段的工作内容。

- 结合本阶段 `.trellis/workspace/` 中的 journal 记录和刚被归档的所有工单，撰写一份复盘总结报告。
- **存放位置**：直接写入 `.docs/design/planning/`，命名为 `[MILESTONE] vX.X-阶段总结-YYYY-MM.md`。

#### 里程碑报告模板

```markdown
# [MILESTONE] vX.X 阶段总结 — YYYY-MM

> 归档时间：YYYY-MM-DD
> 版本跨度：vX.X.0 → vX.X.N
> 核心主题：{一句话概括本阶段主题}

---

## 一、版本发布

| 版本   | 日期       | 类型     | 核心内容 |
| ------ | ---------- | -------- | -------- |
| vX.X.0 | YYYY-MM-DD | feat/fix | {描述}   |

## 二、完成的工单

| 工单 | 标题   | 归档路径                                                   |
| ---- | ------ | ---------------------------------------------------------- |
| T0xx | {标题} | [task.md](.trellis/tasks/archive/YYYY-MM/T0xx-.../task.md) |

## 三、完结的设计文档

| 文档          | 路径                                               |
| ------------- | -------------------------------------------------- |
| [DONE] {名称} | [设计文档](.docs/design/features/[DONE] {名称}.md) |

## 四、关键决策

1. **{决策标题}** — {理由和结论}

## 五、遗留事项

1. {未完成项及其当前状态}
```

#### 路径引用规则

- **优先指向归档文件**（归档路径稳定不变）
- **必须指向动态文件时**，添加备注：`⚠️ 动态路径，若找不到请搜索关键词 "xxx"` 或 `⚠️ 若路径失效，请询问用户`
- 使用相对路径（相对于项目根目录）

## 结束与输出

向用户输出**标准归档统计表**：

```markdown
| 类别         | 动作                       | 数量     |
| ------------ | -------------------------- | -------- |
| design/ 打标 | [DONE] / [OBSOLETE]        | X 个     |
| notes/ 去水  | 剪切已完成事项             | X 条     |
| 冷藏区转移   | dep-skills / dep-workflows | X 个     |
| .tmp/ 清空   | 删除                       | X 个文件 |
| 工单归档     | → archive/YYYY-MM/         | X 个     |
| 里程碑总结   | 已生成                     | 1 份     |
```

通知用户里程碑总结报告已生成，请检阅。
