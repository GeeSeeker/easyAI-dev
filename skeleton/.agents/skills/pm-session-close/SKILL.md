...
5. **用户确认后执行**
   - 用户确认标记 `[DONE]` → 在文档头部添加状态标注（如 `<!-- status: DONE -->`），**不自动重命名文件**（重命名由 `pm-milestone-archive` 统一处理）
   - 用户确认标记 `[OBSOLETE]` → 同上，添加状态标注
   - 用户拒绝 → 跳过

> **注意**：本步骤仅做状态标注，不执行文件重命名或移动操作。文件重命名（加 `[DONE]` 前缀）和归档操作统一由 `pm-milestone-archive` 在里程碑归档时处理。
