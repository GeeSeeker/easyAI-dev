...
skill_chain:
  ...
  conditional_branches:
    - worker-debug
    - worker-five-dimension-audit # PM 创建专项审计任务时激活
...
files:
  ...
  - path: .agents/skills/worker-five-dimension-audit/SKILL.md
    role: 五维深度审计（存量代码全量扫描、跨平台交叉验证、调用链追踪）
...
- **worker-five-dimension-audit**：PM 创建专项审计任务（约束集含 `[AUDIT]` 标记）时激活。存量代码全量扫描、跨平台交叉验证、调用链追踪，产出结构化审计报告到 `.docs/audit/`。
