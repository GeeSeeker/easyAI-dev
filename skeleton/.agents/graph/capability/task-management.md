...
PM 创建任务时的风险评估与审查策略分配。基于文件范围和任务性质判定风险等级（🟢低 / 🟡中 / 🔴高 / ⚫极高），匹配对应审查策略（self_review / single_cli_review / dual_cli_review / dual_cli_plus_internal），从 `config.yaml` 的 `team.roster` 中选择审查者。
